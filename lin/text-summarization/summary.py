...
def summary(text,stop_word,topK_ratio):

    # 按标点符号划分句子
    sentence_set, sentence_with_index = split_sentence(text, punctuation_list='!?。,，！？')
    #print('len_ss', len(sentence_set))
    #print('ss:', sentence_set)
    #print('swi:', sentence_with_index)
    ...
    #print('swww', sentence_with_words_weight)
    sentence_with_position_weight = get_sentence_with_position_weight(sentence_set)
    #print('swpw:', sentence_with_position_weight)
    sentence_score = get_similarity_weight(tfidf_matrix)
    #print('ss', sentence_score)
    sort_sent_weight = ranking_base_on_weigth(sentence_with_words_weight,
                                              sentence_with_position_weight,
                                              sentence_score, feature_weight=[5,1,4])
    summarization = get_summarization(sentence_with_index, sort_sent_weight, topK_ratio)
...
if __name__ == '__main__':
  ...
  #  print(text)
   # print(list(text))
    #print(list(text[0:21 + 1]).pop())
    print('len_text:',len(text))
    stop_word = []
    with open('stopWordList.txt','r') as f:
        for line in f.readlines():
            stop_word.append(line.strip())
    summarization = summary(text,stop_word)
    print('summarization:\n',summarization)
    print(len(summarization),type(summarization))
    file_handle = open('output.txt', mode='w')
    file_handle.write(summarization)